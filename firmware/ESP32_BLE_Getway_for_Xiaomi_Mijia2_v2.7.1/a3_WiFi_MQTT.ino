...
void Connect_WiFi_MQTT() {
  ...
  // Если при старте ESP: FS отформатирована и создался конфиг / ошибка открытия конфига / пустые строки с SSID или MQTT, то...
  if (stat == FD_WRITE || stat == FD_FILE_ERR || String(portalCfg.ssid) == "" || String(portalCfg.loginMQTT) == "") {
#if (DEBUG == 1)  // Выполняется, если отладка включена
  ...
  MDNS.begin(M_DNS);  // Запускаем mDNS
...
void onConnectionEstablished() {  // Возвращает количество раз, когда onConnectionEstablished вызывался с самого начала.
  ...
  String subnMsk = String(subnetMsk_Dec);                                                                                         // Конвертируем из Char в String
  ...
  // Эти публикации будут статическими (отправляются единожды после успешного подключения), true - последние данные не будут стираться после переподключения клиента к серверу
  client.publish("ESP32/SYS/Local IP", ip_addr, true);               // Публикуем IP адрес ESP
  client.publish("ESP32/SYS/mDNS", String(M_DNS) + ".local", true);  // Публикуем адрес mDNS в локальной сети
